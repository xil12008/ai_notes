...
$\mathbb{E}_{\tau} [ R(\tau)] \= \int_{\tau} P(\tau) R(\tau)d\tau$

where $\tau$ is generated by a policy $\pi_\theta$ with the probability $P(\tau) = \prod_{i=0}^{\inf} \pi_{\theta}(a_i | s_i) P(s_{i+1} | s_i, a_i)$
